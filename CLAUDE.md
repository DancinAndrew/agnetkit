...
> **提案之前**，若設計還很模糊，執行 `grill-me` 技能——它每次一個問題審問計畫，直到模糊度歸零，再把摘要餵給 `/plan` 或 `/opsx:propose`。在寫任何 code 之前抓到錯誤設計，成本最低。**提案之後、開工之前**，可執行 `adversarial-spec-review`——以反方角度攻擊寫好的規格，找出模糊、缺漏、不可測的驗收標準，並輸出 spec v2。
